...
CompilerIf Defined(INCLUDE_GOSCINTILLA, #PB_Constant)=0
  ...
  Procedure GOSCI_SetLineBookmark(id, lineIndex, flag=#True, nbbookmark = #GOSCI_BOOKMARKMARKERNUM)
    If IsGadget(id) And GadgetType(id) = #PB_GadgetType_Scintilla
      If flag
        ScintillaSendMessage(id, #SCI_MARKERADD, lineIndex, nbbookmark)    
      Else
        ScintillaSendMessage(id, #SCI_MARKERDELETE, lineIndex, nbbookmark) 
      EndIf
    EndIf
  EndProcedure
  ;/////////////////////////////////////////////////////////////////////////////////
  ...
  ; Load Text
  ;
  Procedure.l GOSCI_LoadText(id,filename.s)
    Protected rfh,bufstr.s = ""
    rfh = ReadFile(#PB_Any,filename.s)
    If rfh
      While Eof(rfh) = 0
        bufstr = bufstr + ReadString(rfh) + Chr(10)
      Wend
